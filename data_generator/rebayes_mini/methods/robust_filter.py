...
class WeightedObsCovFilter(KalmanFilter):
    ...
    def init_bel(self, mean, covariance, key=314):
        state = WOCFState(
            mean=mean,
            cov=jnp.eye(len(mean)) * covariance,
            weighting_term=1.0,
...
class RobustStFilter(ExtendedKalmanFilter):
    ...
    def init_bel(
        self, mean, cov, obs_cov_scale, obs_cov_dof,
...
class ExtendedKalmanFilterSQ(ExtendedKalmanFilter):
    def __init__(
        ...
        # self.max_obs_cov = jnp.max(jnp.linalg.eig(self.observation_covariance))

    def _update(self, bel, y, x):
        err = y - self.vobs_fn(bel.mean, x)
        weighting_term = 1 / (jnp.square(jnp.inner(err, err)))
        # weighting_term = self.soft_threshold

        Ht = self.jac_obs(bel.mean, x)
        Rt = self.observation_covariance + 1/2*weighting_term * jnp.eye(len(y))
